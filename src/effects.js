...
class RainbowEffect {
  ...
  render() {
    if (useparticles) {
      this.particles.push(new RainbowParticle(this.x, this.y, this.vx, this.vy));
...
class RainbowParticle {
  constructor(x, y, vx, vy) {
    ...
    this.vy = random(-20, 20) + vy;
    this.alpha = 255;
    this.randcolor = color(random(0, 255), random(0, 255), random(0, 255))
    this.d = 16;
  }
  ...
  show() {
    if (useparticles) {
      ...
      push()
      noStroke();
      fill(this.randcolor, this.alpha);
      ellipse(this.x, this.y, this.d);
      pop()
    }
  }
